...
const SIDEBAR_WIDTH = "clamp(17rem, 25vw, 20rem)";
...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(
    () => defaultOpen ?? getSavedSidebarState(),
  );
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>Nawigacja aplikacji.</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  const label = isOpen ? "Ukryj panel boczny" : "Pokaż panel boczny";
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const label =
    state === "expanded" ? "Ukryj panel boczny" : "Pokaż panel boczny";
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button relative flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] motion-reduce:transition-none group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mr-0 ml-4 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border py-1 pr-0 pl-4 group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
    />
...
export {
  ...
  SidebarSeparator,
  SidebarTrigger,
  // The shadcn sidebar intentionally exports its context hook beside components.
